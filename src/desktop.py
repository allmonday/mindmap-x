...
def _acquire_single_instance_lock() -> bool:
    ...
    try:
        if sys.platform == "win32":
            # fcntl 是 Unix 专属；Windows 用 msvcrt.locking 做同款非阻塞文件锁
            # （锁 1 字节；持有 _lock_file 引用到进程退出，语义与 flock 一致）
            import msvcrt
...
def main() -> None:
    ...
    os.environ.setdefault("CHAT_PROVIDER_FILE", str(DATA_DIR / "provider.json"))  # UI 配置的网关固化文件
