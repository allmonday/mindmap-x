...
export const api = {
  ...
  updateNode: (node_id: number, content?: string, collapsed?: boolean) =>
    call<NodeDTO>('update_node', { node_id, content, collapsed, actor: 'human' }),
  moveNode: (node_id: number, new_parent_id: number, position?: number) =>
    call<NodeDTO>('move_node', { node_id, new_parent_id, position, actor: 'human' }),
  deleteNode: (node_id: number) => call<boolean>('delete_node', { node_id, actor: 'human' }),
